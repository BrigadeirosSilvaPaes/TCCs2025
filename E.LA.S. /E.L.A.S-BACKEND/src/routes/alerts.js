...
// POST /api/alerts - Criar alerta para usuário logado (COM AUTENTICAÇÃO)
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    const alert = await Alert.create({
      ...
      tipo_alerta,
      numero_medida: req.user.numero_medida,
...
// POST /api/alerts/public - Criar alerta sem autenticação (para emergências)
router.post('/public', async (req, res) => {
  try {
    ...
    console.error('Erro ao registrar alerta público:', error);
    res.status(500).json({
      success: false,
      message: 'Erro ao registrar alerta: ' + error.message
    });
...
// GET /api/alerts/user - Buscar alertas do usuário logado (COM AUTENTICAÇÃO)
...
// GET /api/alerts/user/:usuario_id - Buscar alertas por ID de usuário (para admin)
...
// PUT /api/alerts/:id/resolve - Resolver alerta (COM AUTENTICAÇÃO)
router.put('/:id/resolve', authMiddleware, async (req, res) => {
  try {
    const alert = await Alert.findOne({
      where: { 
        id: req.params.id,
        usuario_id: req.user.id // Garantir que é dono do alerta
      },
      include: [{
        model: User,
        attributes: ['nome', 'nome_social', 'telefone']
...
// PUT /api/alerts/:id/resolve-public - Resolver alerta sem autenticação
router.put('/:id/resolve-public', async (req, res) => {
  try {
...
// GET /api/alerts/ - Buscar todos os alertas (para admin)
router.get('/', async (req, res) => {
  try {
    ...
    const { count, rows: alerts } = await Alert.findAndCountAll({
      ...
      include: [{
        model: User,
        attributes: ['nome', 'nome_social', 'telefone', 'cpf']
      }],
      order: [['criado_em', 'DESC']],
      limit: parseInt(limit),
      offset: offset
